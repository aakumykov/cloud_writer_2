    ...
    : CloudWriter2
{
    ...
    override fun createDeepDir(path: String, isRelative: Boolean): String {
        return if (isRelative) createRelativeDeepDir(path)
        else createAbsoluteDeepDir(path)
    }

    private fun createRelativeDeepDir(path: String): String {
        return createAbsoluteDeepDir(virtualRootPlus(path))
    }

    private fun createAbsoluteDeepDir(path: String): String {
        return iterateDeepOnPathWithAction(path) { partialPath ->
            createAbsoluteDir(partialPath)
        }
    }
    ...
    private fun iterateDeepOnPathWithAction(path: String, action: (String) -> Unit): String {
        return path
            ...
            .reduce { acc, s ->
                createAbsoluteDir(acc)
                acc + CloudWriter2.DS + s
            }.also { tailDir: String ->
                createAbsoluteDir(tailDir)
            }
    }
